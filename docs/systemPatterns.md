...
- **Ingestion Engine**: Listens to folders (`FileWatcher`), chunks semantic data (`SemanticChunker`), and extracts citations/concepts via LLMs.
...
- **Embeddings**: Local LLM gateway generates embeddings, which are consumed by LangChain4j vectors for semantic searches.
